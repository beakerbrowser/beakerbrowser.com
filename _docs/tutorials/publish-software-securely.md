...
One of Beaker’s most powerful features comes from the Dat protocol’s approach to ensuring archive/site integrity. As explained in [Inside Beaker: Privacy and security], each update to a Dat is signed, and and each file is hashed.
...
To do this, we suggest [Hosting the binaries outside of Beaker], perhaps using the [Dat CLI](https://github.com/datproject/dat) or [DatHTTPD](https://github.com/beakerbrowser/dathttpd). Using the latter, your users will be able to download either from dat directly, or over HTTPS if they don’t need the added security.